...
- [`backup`](scripts/backup) - The main script. It can start and stop backups, and it will keep running until it's done. It's basically just a wrapper for [restic](https://restic.net/) with special features like notifications, logging, and a single-instance lock.
...
- [`backup-status`](polybar/scripts/backup-status) - An indicator for polybar that shows if a backup is running, finished, failed, or out of date. It might need some modification to work with other status bars. Here's the important part of the polybar module:
  ```ini
  [module/backup]
  type = custom/script
